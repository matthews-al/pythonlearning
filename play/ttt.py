...
def display_board(board, legend=False):
...
def check_wins(board):
    ...
    returns winning symbol if there's a winner,
     ' ' if there's no winner,
     't' if the game has completed in a tie.
    Note:  I'm sure there are some clever ways to loop through the rows and columns,
     vs. hard coding the sets, however, for this scale, it would likely be overly complex
    """
    # Check rows:
...
def game_loop():
    ...
    while playing:
        ...
        # Do this so we can rely on board[0] being ' '
        if selection > 0: board[selection] = players[curplayer]
